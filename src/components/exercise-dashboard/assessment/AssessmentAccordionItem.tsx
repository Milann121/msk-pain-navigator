...
export const AssessmentAccordionItem = ({
  ...
  const {
    latestPainLevel,
    diffIcon
  } = useLatestPainLevel(assessment.id, assessment.initial_pain_level ?? null);
  ...
  const programStartDate: Date = assessment.program_start_date ? new Date(assessment.program_start_date) : new Date(assessment.timestamp);
  ...
  return <AccordionItem key={assessment.id} value={assessment.id}>
      ...
      <AccordionContent className="px-4 pb-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          {/* Assessment details */}
          <AssessmentDetails assessment={assessment} latestPainLevel={latestPainLevel} diffIcon={diffIcon} />
          <div className="flex flex-col h-full justify-start">
            ...
            <div className="flex flex-col gap-0 mt-2">
              ...
              <AssessmentProgramControls isEnded={isEnded} loadingEnd={loadingEnd} loadingRenew={loadingRenew} onEnd={handleEndProgram} onRenew={handleRenewProgram} />
            </div>
          </div>
        </div>
        <AssessmentAccordionActions assessment={assessment} programEndedAt={isEnded ? programEndedAt ?? new Date() : null} loadingEnd={loadingEnd} loadingRenew={loadingRenew} onOpenFollowUp={onOpenFollowUp} onDeleteAssessment={onDeleteAssessment} handleEndProgram={handleEndProgram} handleRenewProgram={handleRenewProgram} handleViewExercises={handleViewExercises} />
      </AccordionContent>
      
      {/* Program Ending Questionnaire Dialog */}
      <ProgramEndingDialog isOpen={showEndingQuestionnaire} onOpenChange={handleCloseEndingQuestionnaire} selectedAssessment={assessment} onComplete={handleEndingQuestionnaireComplete} />
    </AccordionItem>;
};