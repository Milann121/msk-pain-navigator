...
export const ProfileFormGoals: React.FC<ProfileFormGoalsProps> = ({
  ...
  return <div>
      <h3 className="mb-3 text-blue-800 text-base font-semibold">{t('profile.profileForm.goalsTitle')}</h3>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <Select value={data.weeklyExerciseGoal?.toString() || ""} onValueChange={value => onChange('weeklyExerciseGoal', parseInt(value))}>
            <SelectTrigger className="w-20 h-8">
              <SelectValue placeholder="-" />
            </SelectTrigger>
            <SelectContent>
              {exerciseOptions.map(option => <SelectItem key={option} value={option.toString()}>
                  {option}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={data.weeklyBlogGoal?.toString() || ""} onValueChange={value => onChange('weeklyBlogGoal', parseInt(value))}>
            <SelectTrigger className="w-20 h-8">
              <SelectValue placeholder="-" />
            </SelectTrigger>
            <SelectContent>
              {blogOptions.map(option => <SelectItem key={option} value={option.toString()}>
                  {option}
                </SelectItem>)}
            </SelectContent>
          </Select>
