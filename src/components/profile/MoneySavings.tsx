...
export const MoneySavings = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      const exerciseChannel = supabase.channel('exercise_completion_clicks').on('postgres_changes', {
        event: '*',
        schema: 'public',
      ...
      const assessmentChannel = supabase.channel('user_assessments').on('postgres_changes', {
        event: '*',
        schema: 'public',
  ...
  const calculateSavings = async () => {
    ...
    try {
      ...
      const {
        ...
        error: exerciseError
      } = await supabase.from('exercise_completion_clicks').select('clicked_at').eq('user_id', user.id).eq('is_active', true);
      ...
      } = await supabase.from('user_assessments').select('id').eq('user_id', user.id);
  ...
  return <div className="h-full w-full flex flex-col items-center justify-center">
      ...
      <p className="text-base text-green-600 text-center mx-0 px-[32px]">
