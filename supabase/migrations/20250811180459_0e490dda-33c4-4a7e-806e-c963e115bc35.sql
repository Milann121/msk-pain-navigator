...
create unique index if not exists community_posts_user_week_unique
...
-- RLS policies
create policy if not exists "Anyone authenticated can read community posts"
  on public.community_posts
  for select
  to authenticated
  using (true);

create policy if not exists "Users can insert their own community post"
  on public.community_posts
  for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own community post"
  on public.community_posts
  for update
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own community post"
  on public.community_posts
  for delete
  to authenticated
  using (auth.uid() = user_id);
...
create trigger if not exists trg_community_posts_updated
